        ...
        for ($i=0; $i < count($_SESSION['main_cart']); $i++) { 
            if(is_numeric($_SESSION['main_cart'][$i])) {
                ...
                if($getProduct->rowCount() != 0) {
                    ...
                    header("Location: http://elite-jewellery.dp.ua/infofile.php?message=Вы успешно оформили заказ, мы свяжемся с вами в ближайшее время!"); 
                } 
            } 
        }
    
        $_SESSION['main_cart'] = array();
        unset($_SESSION['main_cart']);
        $_SESSION['cart_price'] = 0;
        $_SESSION['cart_products_rows'] = 0;
    } else {
        header("Location: http://elite-jewellery.dp.ua/infofile.php?message=Вы ничего не заказали!"); 
    }
}
